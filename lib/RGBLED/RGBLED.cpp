...
void RGBLED::setMode(uint8_t MODE){
	...
	digitalWrite(buzzer,HIGH);
	_MODE=MODE;
	Serial.printf("RGBLED mode set to %i\n", MODE);
	previousTime = 0;
	flashIndex = 0;
}
...
void RGBLED::loop(){
	switch(_MODE){
		...
		case 11:{
			if(previousTime == 0){
				if(MODE_SERVER_CONNECTED_INDEX_STATE[flashIndex] == 0){
					turnOff();
					if(_buzzerEnabled){
						digitalWrite(buzzer, HIGH);
					}
				}else{
					writeRGB(MODE_SERVER_CONNECTED_COLOR[0], MODE_SERVER_CONNECTED_COLOR[1], MODE_SERVER_CONNECTED_COLOR[2]);
					if(_buzzerEnabled){
						digitalWrite(buzzer, LOW);
					}
				}
				...
				if(millis() > previousTime+MODE_SERVER_CONNECTED_SEQUENCE[flashIndex]){
					(flashIndex >= (MODE_SERVER_CONNECTED_SIZE-1))? flashIndex = 0 : flashIndex++;
					previousTime = millis();
					if(MODE_SERVER_CONNECTED_INDEX_STATE[flashIndex] == 0){
						turnOff();
						if(_buzzerEnabled){
